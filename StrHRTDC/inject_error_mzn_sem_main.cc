...
int main(int argc, char* argv[])
{
  if(1 == argc){
    std::cout << "Usage\n";
    std::cout << "inject_error_mzn [IP address] [Mezzanine existence]" << std::endl;
  ...
  if(enslot_up){
    MZN::XilinxSEM::InjectError(fpga_module, BBP::kUpper, 100, 10, 1);
  }else{
  ...
  if(enslot_low){
    MZN::XilinxSEM::InjectError(fpga_module, BBP::kLower, 100, 10, 1);
  }else{
    std::cout << "Slot is disabled." << std::endl;
  }
